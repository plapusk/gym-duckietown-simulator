...
class LaneFollower:
    ...
    def set_intersection_direction(self, direction, turn_duration=100):
        """Set this from external input: 'left', 'right', or 'straight'."""
        self.turn_direction = direction
        self.turn_frames_remaining = turn_duration
        self.intersection_mode = True

    def lane_following_control(self, seg_mask, bboxes=None):
        H, W = seg_mask.shape
        ...
        # --- Pedestrian detection on bottom 4 scan lines ---
        if bboxes is not None:
            # Bottom 4 scan lines y range
            bottom_4_y_min = scan_lines[-12]
            bottom_4_y_max = scan_lines[-1]

            # Check for any bbox class 0 overlapping vertical region of bottom 4 scan lines
            pedestrian_present = False
            for bbox in bboxes:
                # bbox format assumed [x1, y1, x2, y2, class_id (int or str)]
                x1, y1, x2, y2 = bbox[:4]
                class_id = int(bbox[4]) if len(bbox) > 4 else None

                if class_id == 0:  # pedestrian class
                    # Check vertical overlap
                    if (y2 >= bottom_4_y_min) and (y1 <= bottom_4_y_max):
            ...
            if pedestrian_present:
                debug_info = {
                    ...
                    "right_danger": False,
                    "left_danger": False,
                    "x_vertical_line": None,
                    "x_target": None,
                    "steering": 0.0,
                    "forward_speed": 0.0,
                    "info": "Waiting for pedestrian",
                    "intersection": False,
                }
                return [0.0, 0.0], 0.0, debug_info

        # === Intersection Detection ===
        bottom_line = int(H * 0.95)
        intersection_pixels = np.sum(seg_mask[bottom_line, :] == 1)
        total_pixels = seg_mask.shape[1]
        has_intersection = (intersection_pixels / total_pixels) >= 0.2

        if has_intersection and not self.intersection_mode:
            debug_info = {
                "scan_lines": scan_lines,
                "mid_lane_xs": [],
                "left_lane_xs": [],
                ...
                "steering": 0.0,
                "forward_speed": 0.0,
                "info": "Waiting at intersection",
                "intersection": True,
            }
            return [0.0, 0.0], 0.0, debug_info

        if self.intersection_mode and self.turn_frames_remaining > 0:
            ...
            if self.turn_frames_remaining == 0:
                self.intersection_mode = False
            ...
            debug_info = {
                ...
                "info": f"Turning {self.turn_direction}",
                "intersection": True,
            }
            return [forward_speed, steering], 0.0, debug_info
...
def main():
    ...
    while not done:
        ...
        raw_rgb_frame = cv2.cvtColor(obs.copy(), cv2.COLOR_RGB2BGR)

        # Resize seg_mask to match frame size
        seg_vis = colorize_segmentation(seg_mask)
        seg_vis_resized = cv2.resize(seg_vis, (640, 640), interpolation=cv2.INTER_NEAREST)

        # Blend segmentation mask overlay with original image
        perception_overlay = cv2.addWeighted(raw_rgb_frame, 0.6, seg_vis_resized, 0.4, 0)

        # Draw bounding boxes on top of the overlay
        for bbox in bboxes:
            # Assuming bbox format: [x1, y1, x2, y2, label(optional)]
            x1, y1, x2, y2 = map(int, bbox[:4])
            label = str(bbox[4]) if len(bbox) > 4 else "Obj"
            cv2.rectangle(perception_overlay, (x1, y1), (x2, y2), (0, 255, 255), 2)
            cv2.putText(perception_overlay, label, (x1, y1 - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

        # Show the combined perception view (original + seg + bboxes)
        cv2.imshow("Perception View", perception_overlay)

        # Run lane following control
        action, last_error, debug_info = follower.lane_following_control(seg_mask, bboxes=bboxes)
        ...
        if debug_info.get("intersection") and debug_info["info"] == "Waiting at intersection":
            ...
            while True:
                ...
                if key == ord('a'):
                    follower.set_intersection_direction("left")
                    break
                elif key == ord('d'):
                    follower.set_intersection_direction("right")
                    break
                elif key == ord('w'):
                    follower.set_intersection_direction("straight")
                    break
                elif key == ord('q'):
                    done = True
                    break
            continue  # Skip this step to wait for user input
        ...
        if debug_info.get("right_danger"):
        ...
        if debug_info["x_target"] is not None:
        ...
        # Show error
        cv2.putText(frame, f"Err: {last_error:.2f}" if last_error is not None else "Err: N/A", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)

        # Show intersection info (if applicable)
        if debug_info.get("intersection"):
            cv2.putText(frame, debug_info["info"], (10, 100),
